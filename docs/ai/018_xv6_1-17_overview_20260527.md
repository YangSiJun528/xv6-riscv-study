...
- xv6 book: <https://xv6-guide.github.io/xv6-riscv-book/>의 2장, 3장, 4장, 6장, 7장
...
- 현재 repo의 user-level sleep syscall 이름은 `sleep`이 아니라 `pause`다. kernel primitive는 여전히
  `sleep(chan, lock)`이다.
- `usertrap()`에는 lazy allocation page fault 처리를 위한 `vmfault()` 호출이 보이지만, VM 함수 상세는
  19-20강 범위로 미룬다.
...
caller-saved는 함수 호출 뒤 값이 보존된다고 기대하면 안 되는 register다. 호출한 쪽이 계속 필요하면 호출 전에
직접 저장해야 한다. RISC-V에서는 주로 `ra`, `a0-a7`, `t0-t6`가 여기에 속한다.

callee-saved는 호출받은 함수가 사용했다면 return 전에 원래 값으로 복구해야 하는 register다. RISC-V에서는
`s0-s11`이 대표적이고, `sp`도 함수가 돌아올 때 호출 전 stack 위치로 복구되어야 한다. `swtch.S`는 kernel
thread 재개에 필요한 `ra`, `sp`, `s0-s11`만 `struct context`에 저장한다.

### 주요 CSR

CSR은 Control and Status Register의 약자다. 일반 register처럼 산술 계산에 쓰는 값이 아니라, CPU의 특권
상태와 trap/page table/interrupt 동작을 제어하거나 trap 결과를 확인하는 특수 register다. xv6는
`kernel/riscv.h`의 `r_*()`, `w_*()` helper로 CSR을 읽고 쓴다.
...
라인 번호는 현재 repo 기준이다. 코드가 바뀌면 함수명을 기준으로 다시 찾으면 된다.

| 흐름 | 파일:라인 | 볼 것 |
| --- | --- | --- |
| boot entry | `kernel/entry.S:7` | `_entry`가 hart별 stack을 만들고 `start()` 호출 |
| machine -> supervisor | `kernel/start.c:15` | `mstatus`, `mepc`, `satp`, timer, `tp`, `mret` |
| first kernel init | `kernel/main.c:11` | CPU 0 초기화, 다른 CPU 대기, 마지막 `scheduler()` |
| spinlock acquire/release | `kernel/spinlock.c:22`, `kernel/spinlock.c:46` | atomic exchange/store, interrupt off/on |
| page allocator | `kernel/kalloc.c:27`, `kernel/kalloc.c:69` | `kmem.freelist`와 `kmem.lock` |
| kernel page table | `kernel/vm.c:22`, `kernel/vm.c:75` | direct map, trampoline map, `satp` enable |
| user trap entry/return | `kernel/trampoline.S:22`, `kernel/trampoline.S:101` | `uservec`, `userret` |
| trap C handler | `kernel/trap.c:38`, `kernel/trap.c:101` | `usertrap()`, `prepare_return()` |
| syscall dispatch | `kernel/syscall.c:135`, `user/usys.pl:9` | `a7` syscall 번호, `a0` 반환값 |
| scheduler switch | `kernel/proc.c:425`, `kernel/proc.c:473`, `kernel/swtch.S:9` | `scheduler()`, `sched()`, `swtch()` |
| sleep/wakeup | `kernel/proc.c:543`, `kernel/proc.c:574`, `kernel/sysproc.c:68` | `sleep()`, `wakeup()`, `sys_pause()` |
...
```mermaid
flowchart LR
  A["QEMU loads kernel at 0x80000000"] --> B["_entry - kernel/entry.S"]
  B --> C["hart별 stack 설정"]
  C --> D["start - machine mode"]
  D --> E["mstatus.MPP = S, mepc = main"]
  E --> F["timer/PMP/delegation 준비"]
  F --> G["mret"]
  G --> H["main - supervisor mode"]
  H --> I["init subsystems"]
  I --> J["scheduler"]
```
...
현재 repo에서 `userinit()`은 첫 process를 `RUNNABLE`로 만들고, 그 process가 처음 스케줄될 때
`kernel/proc.c:forkret()`이 `kexec("/init", ...)`를 호출한다. process 생성과 `exec`의 세부 구현은 뒤
강의 범위다.
...
다만 이 매핑은 user code가 직접 쓰라는 뜻이 아니다. `proc_pagetable()`은 두 mapping에 `PTE_U`를 주지
않는다. trap 직후 CPU는 supervisor mode이지만 아직 user page table을 쓰고 있으므로, supervisor code인
`uservec`/`userret`이 이 mapping을 사용할 수 있게 해 두는 것이다. 이 때문에 trap entry/return 중 page
table이 바뀌어도 trampoline code가 계속 실행된다.
...
sequenceDiagram
  ...
  participant V as uservec in trampoline.S
  participant K as usertrap in trap.c
  participant R as userret in trampoline.S
...
flowchart TD
  ...
  F --> G["sched"]
  G --> H["swtch p->context to cpu->context"]
  H --> I["scheduler resumes"]
  I --> J["find RUNNABLE process"]
  J --> K["p->state = RUNNING"]
  K --> L["swtch cpu->context to p->context"]
...
여기서 scheduler는 전용 core가 아니다. `NCPU=8`은 최대 CPU 수이고, 각 CPU(hart)가 자기 `scheduler()`
loop와 `cpu.context`를 가진다. 한 CPU는 자기 scheduler context와 선택된 process의 kernel context 사이를
오간다.

```text
CPU i:
  cpu[i].context  = 이 CPU의 scheduler 실행 흐름

Process A:
  A.context       = A의 kernel 실행 흐름
  A.kstack        = A가 kernel mode에서 쓰는 stack

Process B:
  B.context       = B의 kernel 실행 흐름
  B.kstack        = B가 kernel mode에서 쓰는 stack

전환:
  scheduler on CPU i
    -> swtch(&cpu[i].context, &A.context)
    -> A의 kernel stack/call chain에서 실행

  A가 yield/sleep/exit
    -> swtch(&A.context, &cpu[i].context)
    -> CPU i의 scheduler stack/call chain에서 실행

  scheduler on CPU i
    -> swtch(&cpu[i].context, &B.context)
    -> B의 kernel stack/call chain에서 실행
```

즉 `swtch()`는 CPU를 바꾸는 것이 아니라, 같은 CPU에서 `ra`, `sp`, `s0-s11`을 바꿔 다른 kernel 실행 흐름으로
`ret`하게 만든다.

`p->lock` handoff는 이 부분에서 가장 헷갈리기 쉽다.

```text
scheduler -> process:
  scheduler: acquire(p->lock)
  scheduler: p->state = RUNNING
  scheduler: swtch(cpu.context, p.context)
  process:   swtch에서 돌아온 뒤 release(p->lock)

process -> scheduler:
  process:   acquire(p->lock)
  process:   p->state = RUNNABLE / SLEEPING / ZOMBIE
  process:   swtch(p.context, cpu.context)
  scheduler: swtch에서 돌아온 뒤 release(p->lock)
```

그래서 이 lock은 process가 user mode에서 실행되는 전체 시간을 보호하는 lock이 아니다. process state와
kernel context를 어느 CPU가 책임지는지 넘기는 짧은 구간을 보호한다.
